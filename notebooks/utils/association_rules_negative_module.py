"""
association_rules_negative_module.py

This module implements Negative Association Rule Mining
for social media marketing analytics.

The system discovers patterns that are strongly associated
with LOW engagement posts using the Apriori algorithm.

Main pipeline:
- Compute weighted engagement
- Normalize engagement rate
- Create low engagement target
- Perform feature engineering
- Generate transactions
- Extract negative association rules
"""
...
def create_negative_target(df):
    """
    Creates binary target variable for low engagement posts.

    Low engagement is defined as the bottom 25%
    of engagement rate values.
    """

    threshold = df["engagement_rate"].quantile(0.25)
...
def run_negative_apriori(
    ...
    to discover low engagement patterns.
    """
    ...
    rules = rules[
        rules["consequents"].apply(
            lambda x: x == frozenset({"low_engagement=True"})
        )
    ]
...
def display_negative_rules(rules, top_n=10):
    """
    Displays negative association rules
    in readable format.
    """

    print("\n" + "=" * 100)
    print("TOP NEGATIVE ASSOCIATION RULES")
    print("=" * 100)
...
def generate_negative_association_rules(df):
    """
    Executes the complete negative association
    rule mining pipeline.
    """

    df = compute_engagement(df)

    df = create_negative_target(df)
    ...
    rules = run_negative_apriori(transactions)
