...
def fallback_benchmark_texts(
    ...
    logger.info("Benchmark insight fallback usage: using local benchmark dashboard text.")
    return {
...
def generate_benchmark_dashboard_texts(
    ...
    if OpenAI is None or not os.getenv("OPENAI_API_KEY"):
        logger.info("Benchmark insight fallback usage: OpenAI client or API key unavailable.")
...
"""
    ...
    try:
        ...
        if not sector_insights:
            logger.info("Benchmark insight fallback usage: GPT returned no usable sector insights.")
            sector_insights = fallback_sector_insights

        logger.info("Benchmark insight generation success via GPT.")
        return {
        ...
        logger.warning("Benchmark GPT generation failed; using local fallback: %s", error)
        return fallback_benchmark_texts(
            context,
            fallback_summary,
            fallback_kpi_insights,
