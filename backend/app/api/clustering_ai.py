...
from app.services.content_style_llm import generate_content_style_ai_insights
...
@router.get("/analyze-ai", response_model=ContentStyleClusteringResponse)
def analyze_content_style_clusters_ai(
    file_path: str = Query(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to parse {suffix or 'dataset'} file: {e}",
        )

    model_path = (
        _project_root / "notebooks" / "clustering" / "artifacts"
        / "content_style_clustering" / "models"
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Missing required column(s): {e}",
        )
    except FileNotFoundError as e:
        raise HTTPException(
            status_code=500, detail=f"Model file not found: {e}",
        )
    except Exception as e:
        raise HTTPException(
    ...
    for r in results:
        ...
        high_cluster_post_count = next(
            (c["post_count"] for c in all_clusters if c["cluster"] == high_c), 0,
        )
        low_cluster_post_count = next(
            (c["post_count"] for c in all_clusters if c["cluster"] == low_c), 0,
        )
        ...
        try:
            insights = generate_content_style_ai_insights(
                business_name=r["business_name"],
                low_cluster=low_cluster_stats,
                high_cluster=high_cluster_stats,
                all_clusters=all_clusters,
            )
            low_meaning = insights["low_meaning"]
            high_meaning = insights["high_meaning"]
            recommendation_text = insights["recommendation"]
        except RuntimeError as exc:
            low_meaning = f"Cluster {low_c}"
            high_meaning = f"Cluster {high_c}"
            recommendation_text = (
                f"(AI unavailable: {exc})\n\n"
                f"Low cluster {low_c}: eng_rate={_get(r['low_performing'], 'avg_engagement_rate', 0):.6f}, "
                f"view_rate={_get(r['low_performing'], 'avg_view_rate', 0):.6f}\n"
                f"High cluster {high_c}: eng_rate={_get(r['high_performing'], 'avg_engagement_rate', 0):.6f}, "
                f"view_rate={_get(r['high_performing'], 'avg_view_rate', 0):.6f}\n\n"
                f"Action: Do more of what cluster {high_c} does — study its content patterns "
                f"and apply them. Avoid cluster {low_c} patterns."
            )
