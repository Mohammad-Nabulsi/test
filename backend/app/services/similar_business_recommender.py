...
POST_TYPE_VALUES = ["post", "image", "reel", "video", "carousel"]
IDENTITY_FILTER_COLUMNS = [
    "category",
    "business_category",
    "business_type",
    "type",
...
def _build_business_profiles(posts_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in _available_columns(df, IDENTITY_FILTER_COLUMNS):
        aggregations[column] = (column, _mode_or_unknown)
...
def select_weak_business_for_comparison(
    posts_df: pd.DataFrame,
    sector: Optional[str] = None,
) -> Dict[str, object]:
    """
    Selects a low-scoring business that has stronger same-sector peers.
    """
    ...
    if sector is not None:
        profiles = profiles[
            profiles["sector"].astype(str).str.lower() == sector.lower()
        ].copy()

    if profiles.empty:
        raise ValueError(f"No businesses found for sector: {sector}")

    candidates = []

    for _, business in profiles.iterrows():
        stronger_peers = profiles[
            (profiles["business_name"] != business["business_name"])
            & (profiles["sector"].astype(str) == str(business["sector"]))
            & (profiles["success_score"] > business["success_score"])
        ]

        if not stronger_peers.empty:
            item = business.to_dict()
            item["stronger_peer_count"] = int(len(stronger_peers))
            item["best_peer_success_score"] = float(stronger_peers["success_score"].max())
            candidates.append(item)

    if not candidates:
        raise ValueError(
            "No weak business with stronger same-sector peers was found."
        )

    return sorted(
        candidates,
        key=lambda item: (
            float(item.get("success_score", 0) or 0),
            -int(item.get("stronger_peer_count", 0) or 0),
        ),
    )[0]
...
def find_similar_successful_businesses(
    ...
    sector: Optional[str] = None,
    top_n: int = 10,
    min_success_percentile: float = 0.45,
) -> Tuple[pd.DataFrame, Dict[str, object]]:
    """
    Finds the nearest similar successful businesses using KNN.

    Similarity pipeline:
    1. Keep only businesses from the same sector
    2. Keep only peers that outperform the target
    3. Run KNN similarity search among those stronger peers
    4. Generate benchmarking recommendations
    """

    profiles = _build_business_profiles(posts_df)

    normalized_target_name = business_name.strip().lower()
    target_filter = profiles["business_name"].astype(str).str.strip().str.lower() == normalized_target_name

    if sector is not None:
        normalized_requested_sector = _normalize_sector_label(sector)
        target_filter &= profiles["sector"].apply(_normalize_sector_label) == normalized_requested_sector
    ...
    logger.info("Total businesses in dataset=%s", len(profiles))

    same_sector = profiles[
        (
            profiles["business_name"].astype(str).str.strip().str.lower()
            != str(target["business_name"]).strip().lower()
        )
        & (
            profiles["sector"].apply(_normalize_sector_label)
            == _normalize_sector_label(target_sector)
        )
    ].copy()
    logger.info("Businesses remaining after same-sector filtering=%s", len(same_sector))
    logger.info("Businesses remaining after removing target business=%s", len(same_sector))

    for column in IDENTITY_FILTER_COLUMNS:
        if column not in same_sector.columns or column not in target.index:
            continue
        target_value = str(target.get(column, "") or "").strip()
        if not target_value or target_value.lower() == "unknown":
            continue
        same_sector = same_sector[
            same_sector[column].astype(str).str.strip().str.lower()
            == target_value.lower()
        ].copy()

    logger.info(
        "Similarity candidate count after sector/category filtering=%s",
        len(same_sector),
    )
    ...
    if candidates.empty:
        return pd.DataFrame(), {
            "ok": False,
            "message": "No same-sector peer businesses are available for comparison.",
    ...
    if peers.empty:
        peers = candidates.sort_values(
            by=["success_score", "business_name"],
            ascending=[False, True],
        ).head(top_n).copy().reset_index(drop=True)
        if not peers.empty:
            peers["knn_distance"] = 1.0
            peers["similarity_score"] = 0.65
            logger.info("KNN returned no peers; using same-sector fallback candidates=%s", len(peers))
        else:
            return pd.DataFrame(), {
                "ok": False,
    ...
    metadata = {
        ...
        "candidate_count_after_sector_filter": int(len(candidates)),
            ...
            [
                {
                    "business_name": target["business_name"],
                    "sector": target["sector"],
                    "recommendation": "Keep testing against higher-scoring similar peers",
                    "reason": "KNN found higher-scoring similar businesses, but no single gap crossed the recommendation threshold. Focus on small tests and consistency.",
                    "your_value": "Similar",
...
def display_similar_business_report(
    ...
    print("\nWhy This Business Can Improve")
    print("-" * 92)
    for explanation in metadata["performance_explanation"]:
        print(f"* {explanation}")

    print("\nNearest Similar Higher-Scoring Businesses")
    print("-" * 92)
    ...
    if peers is None:
        ...
        for index, peer in enumerate(peers, start=1):
            ...
            print(f"   Why it performs better: {peer['outperformance_reason']}")
...
def main() -> None:
    """
    Example run using the processed JSON dataset.
    """

    base_dir = Path(__file__).resolve().parents[2]
    data_path = base_dir / "data" / "data_processed.json"

    posts_df = pd.read_json(data_path)
    example = select_weak_business_for_comparison(posts_df)
    example_business = str(example["business_name"])
